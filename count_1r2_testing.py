...
def analyze_segments(slide_number):
    ...
    for i in range(len(all_roi)):
        ...
        try:
            ...
            dilated_img = cv2.imread(save_path)

            groups = measure.label(patches)
            props = measure.regionprops(groups)
            # print("Number of props = ", len(props))

            for j ,prop in enumerate(props):
                my_centroid = (int(prop.centroid[1]), int(prop.centroid[0]))
                all_centroids.append(my_centroid)
                dilated_img = cv2.circle(dilated_img, my_centroid, 5, (0,0,255), -1)
            
            save_path = path + roi_name + "_dilated_centroids.png"

            cv2.imwrite(save_path, dilated_img)
            
            num_1r2 = groups.max()
            # print("\nNumber of 1R2 clusters = ", num_1r2)
        
        except:
            # print()
            num_1r2 = 0
            # print("Cannot group 1R2. Maybe there are none?")
...
def plot_all_centroids_all_segments(all_centroids, slide_number):
    
    import numpy as np
    
    path = roi_1r2_dir + str(slide_number) + "\\"
    all_roi = [file for file in os.listdir(path) if "dilated" in file and "centroids" not in file]

    centroid_colors = []
    
    # generate unique colors for centroids
    for i in range(len(all_centroids)):
        centroid_colors.append((np.random.randint(254), np.random.randint(254), np.random.randint(254)))
    ...
    for i in range(len(all_roi)):
        ...
        for j in range(len(all_centroids)):
            ...
            image = cv2.circle(image, centroid, 5, color, -1)
            
        save_path = path + roi_name + "_centroids_all.png"
...
def plot_all_centroids(all_centroids, slide_number):
    
    import numpy as np
    
    path = roi_1r2_dir + str(slide_number) + "\\"
    
    image = cv2.imread(path + "roi_0.png")
    
    for j in range(len(all_centroids)):
        centroid = all_centroids[j]
        image = cv2.circle(image, centroid, 5, (255,255,255), -1)

    save_path = path + "centroids_all.png"
    cv2.imwrite(save_path, image)
...
def analyze_centroids(all_centroids):
    
    import math
    
    unique_centroids = all_centroids.copy()
    
    # Elimate centroids that are close to each other (likely the same centr)
    for i in range(len(all_centroids)-1):
        ci = all_centroids[i]
        for j in range(i+1, len(all_centroids), 1):
            cj = all_centroids[j]
            distance = math.sqrt( (ci[0]-cj[0])**2 + (ci[1]-cj[1])**2 )
            if distance < 50:
                if cj in unique_centroids:
                    unique_centroids.remove(cj)
...
def plot_unique_centroids(centroids, slide_number):
    
    path = roi_1r2_dir + str(slide_number) + "\\"
    image = cv2.imread(path + "centroids_all.png")
    
    for i in range(len(centroids)):
         image = cv2.circle(image, centroids[i], 5, (0,0,255), -1)
            
    save_path = path + "centroids_unique.png"
...
def main(slide_number):
    ...
    all_centroids = analyze_segments(slide_number)
    plot_all_centroids(all_centroids, slide_number)
    unique_centroids = analyze_centroids(all_centroids)
    plot_unique_centroids(unique_centroids, slide_number)
    
    num_1r2 = len(unique_centroids)
    # print("number of unique centroids = ", num_1r2 )

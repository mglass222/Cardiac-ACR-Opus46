...
def annotate_1r2(slide_number, saved_database_dir):
...
def main(slide_number, saved_database_dir):
    """Count 1R2 foci for ``slide_number``.

    ``saved_database_dir`` is the per-backend directory containing the
    filtered prediction pickle written by ``wsi.diagnose``.
    """

    slide_number = str(slide_number)

    annotate_1r2(slide_number, saved_database_dir)
    segment_image(slide_number)

    max_1r2 = analyze_segments(slide_number)

...
def model_prediction_dict_to_csv(slide_number, saved_database_dir, test_slide_predictions_dir):
    ...
    make_directory(test_slide_predictions_dir)
    outfile = os.path.join(test_slide_predictions_dir, filename + ".csv")
...
def slide_dx_to_csv(slide_dx_dict, filename, slide_dx_dir):
    ...
    try:
        ...
        outfile = os.path.join(slide_dx_dir, filename)
...
def get_png_slide_path(slide_number):
    ...
    for file in files:
        if str(file.split("-")[0]) == str(slide_number):
            png_slide = file
            png_slide_path = os.path.join(cg.PNG_SLIDE_DIR, png_slide)

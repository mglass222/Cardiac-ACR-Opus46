...
from cardiac_acr.utils.util import Time
...
def filter_remove_small_objects(np_img, min_size=64, avoid_overmask=True, overmask_thresh=95, output_type="uint8"):
  ...
  if (mask_percentage >= overmask_thresh) and (min_size >= 1) and (avoid_overmask is True):
    ...
    util.log("Mask percentage %3.2f%% >= overmask threshold %3.2f%% for Remove Small Objs size %d, so try %d" % (
...
def apply_filters_to_image(slide_num, save=True, display=False):
  ...
  """
  t = Time()
  util.log("Processing slide #%d" % slide_num)
  ...
  if save:
    ...
    util.log("%-20s | Time: %-14s  Name: %s" % ("Save Image", str(t1.elapsed()), result_path))
    ...
    util.log("%-20s | Time: %-14s  Name: %s" % ("Save Thumbnail", str(t1.elapsed()), thumbnail_path))

  util.log("Slide #%03d processing time: %s\n" % (slide_num, str(t.elapsed())))
...
def multiprocess_apply_filters_to_images(save=True, display=False, image_num_list=None):
  ...
  """
  timer = Time()
  util.log("Applying filters to images (multiprocess)\n")
  ...
  util.log("Number of processes: " + str(num_processes))
  util.log("Number of training images: " + str(num_train_images))
  ...
  for num_process in range(1, num_processes + 1):
    ...
    if image_num_list is not None:
      ...
      util.log("Task #" + str(num_process) + ": Process slides " + str(sublist))
    else:
      tasks.append((start_index, end_index, save, display))
      if start_index == end_index:
        util.log("Task #" + str(num_process) + ": Process slide " + str(start_index))
      else:
        util.log("Task #" + str(num_process) + ": Process slides " + str(start_index) + " to " + str(end_index))
  ...
  for result in results:
    if image_num_list is not None:
      ...
      util.log("Done filtering slides: %s" % image_nums)
    else:
      (start_ind, end_ind) = result.get()
      if (start_ind == end_ind):
        util.log("Done filtering slide %d" % start_ind)
      else:
        util.log("Done filtering slides %d through %d" % (start_ind, end_ind))

  util.log("Time to apply filters to all images (multiprocess): %s\n" % str(timer.elapsed()))

...
from cardiac_acr.preprocessing import filter
...
def generate_tile_summaries(tile_sum, np_img, display=True, save_summary=False):
  ...
  """
  # Skip all rendering work when the output would be discarded. Comment out
  # this return to re-enable tile-summary drawing (also requires fonts/arial.ttf).
  if not display and not save_summary:
    return
...
def generate_top_tile_summaries(tile_sum, np_img, display=True, save_summary=False, show_top_stats=True,
  ...
  """
  # Skip all rendering work when the output would be discarded. Comment out
  # this return to re-enable top-tile-summary drawing (also requires fonts/arial.ttf).
  if not display and not save_summary:
    return
  z = 300  # height of area at top of summary slide
  slide_num = tile_sum.slide_num

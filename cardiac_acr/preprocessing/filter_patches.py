...
def filter_green_channel(np_img, green_thresh=200, avoid_overmask=True, overmask_thresh=90, output_type="bool"):
  ...
  if (mask_percentage >= overmask_thresh) and (green_thresh < 255) and (avoid_overmask is True):
    ...
    util.log("Mask percentage %3.2f%% >= overmask threshold %3.2f%% for Remove Green Channel green_thresh=%d, so try %d" % \
...
def multiprocess_apply_filters_to_images(folder, save=False, display=False, image_num_list=None):
  ...
  util.log("Applying filters to Patches (multiprocess)\n")
  ...
  util.log("Number of images: " + str(num_images))
  ...
  for num_process in range(1, num_processes + 1):
    ...
    if image_list is not None:
      ...
      util.log("Task # " + str(num_process) + " process " + str(len(sublist)) + " images")
  ...
  for result in results:
    if image_list is not None:
      ...
      util.log("Done filtering images")
